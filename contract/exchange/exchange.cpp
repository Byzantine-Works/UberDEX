...
void exchange::userwithdraw(account_name user, extended_asset quantity, uint64_t nonce)
{
  ...
  _accounts.adjust_balance_on_user_withdrawal(
      user, -quantity, tapos_block_num(), exchange_settings.funds_release_period, nonce);
  currency::inline_transfer(_self, user, quantity, "withdraw");
  print("exchange.userwithdraw from the exchange, to ", name{user}, "\n");
}

/// @abi action
void exchange::getbalances(account_name owner)
{
  _accounts.get_balances(owner, tapos_block_num());
}
...
void exchange::resetex(account_name owner)
{
  ...
  settings_object.remove();
  settings_object.get_or_create(_self);
}
                     ...
                     bytes makersig, bytes takersig)
{
  ...
  _accounts.update_order(maker, order_hash, amount, amountbuy, amountsell, makerfee,
                         ...
                         nonce, tapos_block_num(), exchange_settings.trade_lock_period, makersig);
...
void exchange::setlockparam(account_name admin, int64_t tradelockperiod, int64_t fundsreleaseperiod)
{
  // Make sure the account that signed this action is an admin.
  require_auth(admin);
  assertIsAdmin(admin);

  eosio_assert(tradelockperiod >= 0, "Trade lock period must not be negative.");
  eosio_assert(fundsreleaseperiod >= 0, "User funds release period must not be negative.");
  eosio_assert(fundsreleaseperiod <= MAX_USER_FUNDS_LOCKUP_PERIOD_BLOCKS, "User funds lock up period must be reasonable.");

  settings current_settings = settings_object.get_or_create(_self);
  print("BEFORE: state of settings.inactivity_release_prd => ", current_settings.funds_release_period, "\n");

  current_settings.trade_lock_period = tradelockperiod;
  current_settings.funds_release_period = fundsreleaseperiod;
  settings_object.set(current_settings, _self);
  print("Setting user funds release period since the account lock to ", fundsreleaseperiod, " blocks\n");
  print("Setting trade lock period since the account lock to ", tradelockperiod, " blocks\n");
  print("AFTER: state of settings.funds_release_period => ", current_settings.funds_release_period,
        ", settings.trade_lock_period => ", current_settings.trade_lock_period, "\n");
}
...
void exchange::lock(account_name user)
{
  require_auth(user);
  _accounts.lock_user_account(user, tapos_block_num());
}

void exchange::unlock(account_name user)
{
  require_auth(user);
  _accounts.unlock_user_account(user);
}
...
void exchange::apply(account_name contract, account_name act)
{
  ...
  switch (act)
  {
    EOSIO_API(exchange, (withdraw)(userwithdraw)(registeruser)(trade)(getbalances)(resetex)(ispkpaired)(invalorders)(setlockparam)(setadmin)(lock)(unlock))
  };
}
} // namespace eosio
