...
struct exaccount
{
    ...
    int64_t last_active_transaction;
    uint64_t invalid_order_nonce;
    uint64_t last_withdrawal_nonce;
    std::vector<char> public_key;

    uint64_t primary_key() const { return owner; }
    EOSLIB_SERIALIZE(exaccount, (owner)(balances)(executed_trades)(order_fills)(last_active_transaction)(invalid_order_nonce)(public_key)(last_withdrawal_nonce))
};

typedef eosio::multi_index<N(exaccounts2), exaccount> exaccounts;
...
struct exchange_accounts
{
    ...
    void get_balances(account_name owner);
