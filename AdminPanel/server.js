...
app.use(session({ secret: 'jamil@active', resave: false, saveUninitialized: true, }));
    ...
    if (error){
      console.log("search error: "+error)
        ...
        //  console.log(response);
         res.render('admin',{data:response.hits});
...
rush= randomNumber(options)+""+rush;
    ...
    res.render('newExchange',{codes:rush});
});
...
transporter.sendMail(mailOptions, function(error, info){
  ...
  setTimeout(function(){res.redirect('/setPassword');},2000);
...
app.get('/appContent/:id',async function(req,res){
    var contents={
        "aboutUs":"","agreement":"","contactUs":"","trade":"","mainTitle":"","mainContent":"","uid":req.params.id
    };
    var footer={
        "footerTitle":"","footerContent":"","footerCopyright":"","footerBottom":""
    };
    var tickers={"hits":""};
    var services={"hits":""};
    var features={"hits":""};
 ...
 body: {
    ...
    if (error){
      ...
      setTimeout(function(){
           ...
           res.render('pageContent',{data:contents,ticker:tickers,footer:footer,services:services,features:features});
...
let arm= await fetch('https://api.byzanti.ne/exchanges?api_key=FQK0SYR-W4H4NP2-HXZ2PKH-3J8797N')
...
let resp = await  client.search({  
 ...
 res.render('admin',{data:resp.hits});
